...
export function PMToolbar({ activeState, trackChangesState, editingMode = "editing", onLink, onImageUpload, onOpenChangesPanel, onModeChange, canEdit: canEditProp, editorApiRef }: Props) {
  ...
  return (
    ...
    >
      ...
      {/* Headings */}
      <Btn title="Heading 1 (Ctrl+Alt+1)" active={blockType === "h1"} style={{ fontWeight: 700, fontSize: "var(--fs-sm)" }} onActivate={run(() => api?.setHeading?.(1))}>H1</Btn>
      <Btn title="Heading 2 (Ctrl+Alt+2)" active={blockType === "h2"} style={{ fontWeight: 700, fontSize: "var(--fs-xs)" }} onActivate={run(() => api?.setHeading?.(2))}>H2</Btn>
      <Btn title="Heading 3 (Ctrl+Alt+3)" active={blockType === "h3"} style={{ fontWeight: 700, fontSize: "var(--fs-xs)" }} onActivate={run(() => api?.setHeading?.(3))}>H3</Btn>
      <Btn title="Heading 4 (Ctrl+Alt+4)" active={blockType === "h4"} style={{ fontWeight: 700, fontSize: "var(--fs-2xs)" }} onActivate={run(() => api?.setHeading?.(4))}>H4</Btn>
