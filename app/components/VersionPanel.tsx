  ...
  return (
    <div style={inline ? inlinePanelStyle : panelStyle}>
      ...
      <div style={headerStyle}>
        <span style={{ fontWeight: 700, fontSize: "var(--fs-md)" }}>Version history</span>
        <button
          onClick={onClose}
          aria-label="Close"
          style={{ background: "none", border: "none", cursor: "pointer", fontSize: "var(--fs-2xl)", opacity: 0.5, padding: "0 2px", color: "var(--fg)" }}
        >×</button>
      </div>
      ...
      <div style={{ flex: 1, overflowY: "auto" }}>
        {historyFetcher.state === "loading" && rows.length === 0 && (
          <div style={{ padding: "1rem 0.75rem", fontSize: "var(--fs-base)", opacity: 0.4 }}>Loading…</div>
        )}
        {historyFetcher.state !== "loading" && rows.length === 0 && (
          <div style={{ padding: "1rem 0.75rem", fontSize: "var(--fs-base)", opacity: 0.4 }}>
        ...
        {rows.length > 0 && (
          ...
          >
            <span style={{ fontWeight: 600, fontSize: "var(--fs-base)" }}>Current version</span>
            ...
            return (
              ...
              >
                <div style={{ minWidth: 0, flex: 1 }}>
                  <span style={{ fontSize: "var(--fs-base)", fontWeight: isSelected ? 600 : 400 }}>
                    {session.userName || "Anonymous"}
                  </span>
                  <span style={{ fontSize: "var(--fs-sm)", opacity: 0.45, marginLeft: "0.4rem" }}>
                    {timeAgo(session.endedAt)}
                  </span>
                </div>
                <div style={{ display: "flex", alignItems: "center", gap: "0.4rem", flexShrink: 0 }}>
                  {isSelected && (
                    <span style={{ fontSize: "var(--fs-2xs)", opacity: 0.55, color: "var(--accent)" }}>preview ↗</span>
                  )}
                  <span style={{ fontSize: "var(--fs-xs)", opacity: 0.45 }}>
            ...
            return (
              ...
              >
                <div style={{ display: "flex", alignItems: "center", gap: "0.4rem", minWidth: 0 }}>
                  <span style={{ fontSize: "var(--fs-base)", fontWeight: isSelected ? 600 : 400, color: "var(--fg)" }}>
                    {timeAgo(version.created_at)}
                  </span>
                  {isVersionLoading && <span style={{ fontSize: "var(--fs-xs)", opacity: 0.5 }}>loading…</span>}
                </div>
                <div style={{ display: "flex", alignItems: "center", gap: "0.4rem", flexShrink: 0 }}>
                  {version.creator_name && !version.auto && (
                    <span style={{ fontSize: "var(--fs-xs)", opacity: 0.55, maxWidth: "6rem", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
      ...
      {selectedVersion && (
        <div style={{ padding: "0.75rem", borderTop: "1px solid color-mix(in srgb, var(--fg) 8%, transparent)", flexShrink: 0 }}>
          ...
          <p style={{ margin: "0.4rem 0 0", fontSize: "var(--fs-xs)", opacity: 0.5, lineHeight: 1.4 }}>
