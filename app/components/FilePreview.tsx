...
export function FilePreviewContent({ file, title }: { file: string; title: string }) {
  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: "1.5rem" }}>
      <div style={{ fontSize: "var(--text-4xl)", opacity: 0.3 }}>
        <svg width="64" height="64" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1" strokeLinecap="round" strokeLinejoin="round">
          <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" />
          <polyline points="14 2 14 8 20 8" />
        </svg>
      </div>
      <span style={{ fontSize: "var(--text-md)", opacity: 0.5 }}>{ext.slice(1).toUpperCase()} file</span>
      <a
        href={url}
        download
        style={{
  ...
  const navActions = (
    <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
      ...
      <a
        ...
        download
        style={{
          fontSize: "var(--text-xs)", opacity: 0.5, textDecoration: "none", color: "var(--fg)",
