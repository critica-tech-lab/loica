  ...
  return (
    ...
    >
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-label={`Move ${itemType === "doc" ? "document" : "folder"}`}
      ...
      >
        <div className="flex items-baseline justify-between">
          <h3 className="text-sm font-bold">
            Move {itemType === "doc" ? "document" : "folder"}
          </h3>
          <button
            onClick={onClose}
            aria-label="Close"
          ...
          {isLoading ? (
            ...
            <>
              ...
              <Form method="post">
                ...
                <input type="hidden" name="targetFolderId" value={rootFolder?.id ?? ""} />
                {isOtherWorkspace && (
                  <input type="hidden" name="targetWorkspaceId" value={selectedWorkspaceId} />
                )}
                <button
                  type="submit"
              ...
              {tree.map(({ folder, depth }) => {
                ...
                return (
                  <Form method="post" key={folder.id}>
                    <input type="hidden" name="intent" value={intent} />
                    <input type="hidden" name={idField} value={itemId} />
                    <input type="hidden" name="targetFolderId" value={folder.id} />
                    {isOtherWorkspace && (
                      <input type="hidden" name="targetWorkspaceId" value={selectedWorkspaceId} />
                    )}
                    <button
