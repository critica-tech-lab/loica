...
import { ChevronRight, HomeIcon, FolderIcon, DocIcon } from "~/components/icons";
...
export default function TeamspaceFolderView() {
  const { workspace, role, folder, folders, documents, folderPath, allFolders, userGroups, starredDocs, directlySharedDocIds, page, pageSize, totalDocs, personalWsId, personalWsName, teamspaces } =
            ...
            <span key={seg.id} className="flex items-center gap-1">
              ...
              <Droppable target={{ type: "folder", id: seg.id }} disabled={!canEdit}>
                {({ dropRef, isOver, isInvalid }) => (
                  <a
                    ref={dropRef}
                    href={`/t/${workspace.id}/folder/${seg.id}`}
                    className={`rounded px-1.5 py-0.5 no-underline transition-all
                      ${isOver && !isInvalid ? "bg-accent/10 text-fg ring-1 ring-accent/30" : ""}
                      ${isInvalid ? "bg-scarlet/5 text-fg ring-1 ring-scarlet/30" : ""}
                      ${!isOver ? "text-fg/50 hover:bg-fg/5 hover:text-fg/80" : ""}
          ...
          {(totalDocs > 0) && (
            <div className="pagination">
              ...
              <div className="flex items-center gap-2">
                {hasPrevPage && (
                  <Link to={`/t/${workspace.id}/folder/${folder.id}?page=${page - 1}`} className="pagination-link">Previous</Link>
                )}
                {hasNextPage && (
                  <Link to={`/t/${workspace.id}/folder/${folder.id}?page=${page + 1}`} className="pagination-link">Next</Link>
                )}
              </div>
            </div>
          )}
            ...
            <div className="flex flex-col items-center justify-center gap-4 py-16 text-center">
              ...
              {canEdit && (
                <div className="mt-2 flex flex-wrap items-center justify-center gap-2">
                  <button
                    type="button"
                    onClick={() => createDocFetcher.submit({ intent: "create-doc" }, { method: "post" })}
        ...
        {confirmAction && (
          <ConfirmModal
            title={
              ...
              confirmAction.type === "unshare-folder" ? "Remove all shares" :
            ...
            message={
              confirmAction.type === "delete-doc"
                ...
                : confirmAction.type === "unshare-folder"
